            ...
            <div className='flex relative flex-wrap w-[100%] gap-x-5'>
                ...
                return(
                    <div key={index} className={`flex mb-3 relative custom-s:h-[13.75rem] flex-grow h-[16.25rem] w-[30rem] custom-xl2:w-[100%]`}>
                        <div className={`flex bg-gray-2E ${index >= 2 ? 'rounded-md':'rounded-b-md'} overflow-hidden relative max-w-full w-full h-full`}>
                            ...
                            <div className='block overflow-auto max-w-full w-[18.5rem] custom-xl2:w-[100%] relative text-rose-50 p-[10px] pb-0 h-full'>
                                <Link href={`${process.env.NEXT_PUBLIC_FRONT_API}/catalog/item/${seriesNames[index]}`} className='inline-block duration-500 transition-colors ease-in-out hover:text-green-400 uppercase absolute ml-[10px] w-fit left-0 right-0 text-[14px] max-w-full font-semibold truncated-text'>{item.SeriesViewName}</Link>
