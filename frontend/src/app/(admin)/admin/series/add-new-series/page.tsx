'use client'
import { newSeries } from "@/components/admin/dto/addNewSeries.dto";
...
import { uploadImage, addSeries, getDataView, handleDeleteEpisode, updateSeries } from "@/utils/admin.logic";
...
export const SeriesInfoSchema = z.object({
    AlternitiveNames: z.array(z.string()),
    SeriesName: z.string(),
    ...
    Shikimori: z.string().nullable(),
    Studio: z.array(z.string()),
    AmountOfEpisode: z.number(),
    VoiceActing: z.array(z.string()),
    CurrentEpisode: z.number().optional(),
    NextEpisodeTime: z.string().optional(),
  });
export type SeriesInfoZod = z.infer<typeof SeriesInfoSchema>;
...
const newSeriesPage = ()=>{
    ...
    const [data,setData] = useState<SeriesInfo | null>({
        AlternitiveNames: [],
        SeriesName: '',
        Description: '',
        SeriesViewName: '',
    ...
    const handleParseData = ()=>{
        const parsedData = SeriesInfoSchema.safeParse(getValues());
        if(parsedData.error){
            console.log(`ERROR: `,parsedData.error);
            console.log(`UNPARSED data: `,getValues());
            return toast.error(`Error: ${parsedData.error.errors[0].message}`);
        }else{
            console.log(`Parsed data: `,parsedData.data);
            console.log(`UNPARSED data: `,getValues());
            
            handleDataSubmit(getValues());
        }
    }
    const handleDataSubmit = (data:SeriesInfoZod)=>{
    ...
    const deleteItem = (type: keyof SeriesInfo, index: number) => {
        if (data && Array.isArray(data[type])) {
            ...
            setData((prev) => prev ? { ...prev, [type]: newArray } : null);
            reset({ ...currentData, [type]: newArray });
        }
    };
    return(
        <div className="flex flex-col max-w-full w-full min-h-full">
            <form className="flex max-w-full w-full max-h-full">
                <div className='flex flex-col relative bg-[#3C3C3C] p-5 w-[68rem] max-w-[96%] max-h-full text-rose-50 rounded-lg flex-wrap'>
                    <div className="flex w-full min-h-[22rem] custom-xl2:flex-col">
                        <UploadPoster seriesName={seriesName}/>
                        <div className='flex flex-col min-h-[22rem]'>
                            <textarea className='text-3xl custom-xs:mt-0 flex bg-transparent min-h-[2.5rem] min-w-[2rem]' placeholder="SeriesViewName" {...register('SeriesViewName')}></textarea>
                            ...
                            <li className="flex">
                                    <div className='flex gap-1 w-[24rem] flex-wrap'>{data?.AlternitiveNames?.map((item:string,index:number)=>(
                                        <div key={index} className="flex bg-gray-100 rounded-custom-sm px-1 pl-2">
                                         ...
                                         <button type="button" onClick={()=>{
                                            ...
                                            if(currentValues.AlternitiveNames === undefined){
                                                setData((prev) => (prev?{...prev,AlternitiveNames:['']}:null));
                                                reset({...currentValues,AlternitiveNames:['']});
                                            }
                                            else{
                                                const updatedAlternitiveNames:string[] = [...currentValues.AlternitiveNames,""];
                                                setData((prev) => (prev?{...prev,AlternitiveNames:updatedAlternitiveNames}:null));
                                ...
                                <li className="flex">
                                    ...
                                    <textarea {...register('SeriesName')} placeholder="SeriesName" className="bg-transparent ml-5 min-w-[2rem] break-words max-w-[30rem]"></textarea>
                                </li>
                                <li className="flex">
                                    <div className='w-[6rem]'>Status:</div>
                                    <textarea className='ml-5 bg-transparent min-w-[2rem]' placeholder="came out" {...register('Status')}></textarea>
                                </li>
                                <li className="flex">
                                    <div className='w-[6rem]'>Shikimori:</div>
                                    <textarea className='ml-5 bg-transparent min-w-[2rem]' placeholder="https://shikimori.one/animes/33-kenpuu-denki-berserk" {...register('Shikimori')}></textarea>
                                </li>
                                <li className="flex">
                                    <div className='w-[6rem]'>Type:</div> 
                                    <textarea className='ml-5 bg-transparent min-w-[2rem]'  {...register('Type')}></textarea></li>
                                <li className="flex">
                                    <div className='w-[6rem]'>Release:</div> 
                                    <textarea className='ml-5 bg-transparent min-w-[2rem]' placeholder="March 2003" {...register('ReleaseYear')}></textarea></li>
                                <li className="flex">
                                    ...
                                    <div className='flex gap-1 ml-5 w-[24rem] flex-wrap'>{data?.Genre.map((item:string,index:number)=>(
                                        <div key={index} className="flex border-2 border-gray-500 bg-transparent rounded-md px-1 pl-[6px]">
                                            <textarea {...register(`Genre.${index}`)} defaultValue={item} className={`flex outline-none bg-transparent hover:border-rose-50 rounded-md font-medium min-w-[2.5rem] max-w-[12rem] break-words text-[0.85rem] py-[2px] items-center mr-1 justify-center`}>
                                        ...
                                        <button type="button" onClick={()=>{/* setGenreItems((prev)=>prev + 1); */
                                            ...
                                            setData((prev) => (prev?{...prev,Genre:updatedGenre}:null));
                                    ...
                                    <div className='flex gap-1 ml-5 w-[24rem] flex-wrap'>{data?.Studio.map((item:string,index:number)=>(
                                        <div key={index} className="flex bg-[#402D9F] rounded-md px-1 pl-[6px]">
                                         ...
                                         <button type="button" onClick={()=>{/* setGenreItems((prev)=>prev + 1); */
                                            ...
                                            setData((prev) => (prev?{...prev,Studio:updatedStudio}:null));
                                ...
                                <li className="flex">
                                    ...
                                    {status === 'ongoing' && (
                                        <textarea className="ml-5 min-w-[1.25rem] bg-transparent" {...register('CurrentEpisode', { valueAsNumber: true })} ></textarea>
                                    )}
                                    {status === 'ongoing' && 'of'}<textarea className={`bg-transparent min-w-[2rem] ${status === 'ongoing' ? 'ml-2':'ml-5'}`} {...register('AmountOfEpisode', { valueAsNumber: true })}></textarea>
                                    {status === 'ongoing' && (
                                        <div className="flex min-w-[6rem] ml-5 items-center">NextEpisodeTime 
                                            ...
                                            <textarea className="bg-transparent min-w-[2rem] ml-2" {...register('NextEpisodeTime')}></textarea>
                                        </div>
                                    )}
                                </li>
                                <li className="flex">
                                    ...
                                    <div className='ml-5 flex gap-1 w-[17rem] flex-wrap'>{data?.VoiceActing.map((item:string,index:number)=>(
                                        ...
                                        <button type="button" onClick={()=>{
                                            ...
                                            setData((prev)=>(prev?{...prev,VoiceActing:updatedVoice}:null));
                    ...
                    <div className='flex relative break-words w-[43.75rem] mt-9 ml-auto mr-auto'>
                        <textarea className='bg-transparent flex items-start justify-start min-w-[2rem]' placeholder="Here should be description :>" {...register('Description')}></textarea>
                    </div>
                    <div className="flex w-full h-[4rem] justify-end">
                        <div className="flex w-[10rem] gap-2 h-full mr-[5rem] items-center font-semibold text-[0.85rem]">
                            ...
                            <button type='submit' onClick={handleSubmit(handleParseData)} className="flex w-[55%] h-[1.75rem] bg-[#5DC090] rounded-md items-center justify-center">
                                submit
                            </button>
                        </div>
                    </div>
            ...
            {/* <div className="flex flex-col w-[68rem] max-w-[96%] min-h-[15rem] mt-5 p-5 bg-gray-300 rounded-lg">
                <div className="flex w-full">
                    <Voices seriesName={} initializeVoiceInfo={voices}/>
                    <div className={`flex ml-4 flex-col box-border h-[1.75rem] relative w-[15rem] font-normal mb-2 text-white text-[0.9rem]`}>
                        ...
                        <div className={`flex ${isShowEpisode?'scale-y-100':'scale-y-0'} w-[15rem] mr-0 -mt-[1px] box-border max-h-[12rem] absolute p-0 m-0 top-full px-2 bg-gray-100 border-gray-600 border-t-0 rounded-b-lg border-[1px] z-10 overflow-y-scroll`}>
                            <div className="flex flex-col w-full gap-y-2">
                                {Array.from({length:data?.AmountOfEpisode || 0},(item:number,index)=>(
